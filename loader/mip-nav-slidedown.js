...
// mip-nav-slidedown/mip-nav-slidedown.js
// ======================
 ...
 * @time 2016.09
 */
define('mip-nav-slidedown/mip-nav-slidedown', [
    'require',
    'customElement',
    'zepto'
], function (require) {
    ...
    function render(me) {
        ...
        var $btnWrap = '<div class="navbar-header">' + '<button class="navbar-toggle collapsed" type="button" data-target="#' + id + '" aria-controls="' + id + '" aria-expanded="false">' + '<span class="sr-only">导航</span>' + '<span class="icon-bar"></span>' + '<span class="icon-bar"></span>' + '<span class="icon-bar"></span>' + '</button>' + (showBrand ? '<a href=' + brandHref + ' class="navbar-brand">' + brandName + '</a>' : '') + '</div>';
        ...
        if ($wiseNav.hasClass('in')) {
            ...
            $(window).on('orientationchange', function () {
                window.setTimeout(function () {
                    // hack: orientationchange 取window高度不及时
                    setNavHeight('resize');
                }, 100);
            }).on('resize', function () {
                setNavHeight('resize');
...
}), define('mip-nav-slidedown', ['mip-nav-slidedown/mip-nav-slidedown'], function (main) {
    return main;
});
